...
// var once sync.Once
// func (instance *spbftCore) a() {
// 	instance.view = instance.view + instance.id * 2
// 	logger.Infof("Replica %d update view to %d", instance.id, instance.view)
// }
...
func (instance *spbftCore) recvViewChange(vc *ViewChange) events.Event {
 ...
 //    if vc.H < instance.h {
 //    	logger.Warningf("Replica %d found vc watermarks is low for sender: %d, begin to sendViewChange", instance.id, vc.ReplicaId)
	...
	if _, ok := instance.viewChangeStore[vcidx{vc.View, vc.ReplicaId}]; ok {
		...
		//if instance.viewChangeSeqNo % 10 == 1{
	    	//logger.Infof("Replica %d  updated instance.viewChangeSeqNo to %d", instance.id, instance.viewChangeSeqNo)
	    	//return nil
	   	//}
		// maxExec := instance.lastExec
	 //   	if instance.currentExec != nil {
...
func (instance *spbftCore) sendNewView() events.Event {
	...
	if _, ok := instance.newViewStore[instance.view]; ok {
		...
		logger.Infof("Replica %d already has new view in store for view %d, skipping", instance.id, instance.view)
		return nil
	}
...
func (instance *spbftCore) processNewView() events.Event {
	...
	if !ok {
		...
		logger.Infof("Replica %d ignoring processNewView as it could not find view %d in its newViewStore", instance.id, instance.view)
		return nil
	}
...
func (instance *spbftCore) processNewView2(nv *NewView, cp ViewChange_C) events.Event {
	...
	// _, ok := instance.chkpts[cp.SequenceNumber]
	// if !ok {
...
func (instance *spbftCore) selectInitialCheckpoint(vset []*ViewChange) (checkpoint ViewChange_C, ok bool, replicas []uint64) {
	...
	if len(checkpoints) == 0 {
		...
		logger.Infof("Replica %d has no checkpoints to select from: %d %s",
			instance.id, len(instance.viewChangeStore), checkpoints)
		return
	}
	...
	for idx, vcList := range checkpoints {
		...
		if len(vcList) <= instance.f { // type casting necessary to match types
			...
			logger.Infof("Replica %d has no weak certificate for n:%d, vcList was %d long",
				instance.id, idx.SequenceNumber, len(vcList))
			continue
		}
		...
		if quorum < instance.intersectionQuorum() {
			...
			logger.Infof("Replica %d has no quorum for n:%d", instance.id, idx.SequenceNumber)
			continue
		}
...
nLoop:
	...
	nullLoop:
		...
		if quorum >= instance.intersectionQuorum() || (len(vset) <= (instance.N - instance.f) && instance.lastNewViewTimeout >= 8) {
			...
			logger.Infof("---id:%d not assign seqno:%d, quorum:%d", instance.id, n, quorum)
		}
